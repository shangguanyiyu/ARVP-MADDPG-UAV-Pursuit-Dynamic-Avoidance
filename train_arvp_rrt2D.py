...
if __name__ == '__main__':
    ...
    STAGE = DEFAULT_STAGE
    print(f"[方案二-RRT2D] 使用 stage = {STAGE}")

    RUN_PREFIX = get_run_prefix(STAGE)
    print(f"[方案二-RRT2D] RUN_PREFIX = {RUN_PREFIX}")

    RUNS_PARENT_DIR = os.path.join("runs", f"{RUN_PREFIX}_arvp_rrt2D")
    os.makedirs(RUNS_PARENT_DIR, exist_ok=True)
    RUNS_UAV_BASE = os.path.join(RUNS_PARENT_DIR, "UAV_training")

    LOG_DIR = os.path.join("log_arvp_rrt2D", f"{RUN_PREFIX}_arvp_rrt2D")
    os.makedirs(LOG_DIR, exist_ok=True)
    print(f"[方案二-RRT2D] 日志目录: {LOG_DIR}/")

    BASE_CHKPT_DIR = os.path.join("tmp_avoid_dynamic", "maddpgwithatt_rrt2D")
    ...
    RUN_CONFIG = {
        ...
        "reward_png_name": f"{RUN_PREFIX}_{STAGE}_arvp_rrt2D_reward.png",
        "obs_agt_dim": 38,
        "obs_tar_dim": 32,
        "n_agents": 4,
        "n_actions": 2,
        "method": "方案二: RRT* 2D 采样路径作为专家轨迹的演示增强 (Demonstration-Augmented RL)",
        "created_at": datetime.now().isoformat(timespec='seconds'),
    }
    cfg_path = save_run_config(RUN_CONFIG)
    print(f"[方案二-RRT2D] 运行参数配置（供 evaluate 读取）已保存: {cfg_path}")

    env = UAVEnv()
    log_dir_tb = create_unique_log_dir(RUNS_UAV_BASE)
    writer = SummaryWriter(log_dir=log_dir_tb)
    print(f"[方案二-RRT2D] TensorBoard runs 目录: {log_dir_tb}/")
    ...
    print(f"[方案二-RRT2D] actor_dims = {actor_dims}, critic_dims = {critic_dims}")
    ...
    with open(hyperparams_txt_path, 'w') as f:
        ...
        f.write(f'beta: {beta}\n')
        f.write(f'RRT* robot_radius: {env.rrt_robot_radius}\n')
        f.write(f'RRT* max_iter: {env.rrt_max_iter}\n')
        f.write(f'RRT* step_size: {env.rrt_step_size}\n')
        f.write(f'RRT* goal_radius: {env.rrt_goal_radius}\n')
        f.write(f'RRT* neighbor_radius: {env.rrt_neighbor_radius}\n')
        f.write(f'RRT* encircle_radius: {env.rrt_encircle_radius}\n')
        f.write(f'RRT* num_waypoints: {env.rrt_num_waypoints}\n')
        f.write(f'mu_rrt (total weight): {env.mu_rrt}\n')
        f.write(f'mu_rrt_track (alignment): {env.mu_rrt_track}\n')
        f.write(f'mu_rrt_progress (progress): {env.mu_rrt_progress}\n')
        f.write(f'capture_milestone_reward: {env.capture_milestone_reward}\n')
        f.write(f'capture_d_milestone_1: {env.capture_d_milestone_1}\n')
        f.write(f'capture_d_milestone_2: {env.capture_d_milestone_2}\n')
        f.write(f'Obs dim agent: 38, target: 32\n')
        f.write(f'reward components: mu1=0.9, mu2=0.2, mu3=0.0(stage1), mu4=10, mu5=0.1, mu_rrt={env.mu_rrt}\n')
    print(f"[方案二-RRT2D] 超参数文本保存到: {hyperparams_txt_path}")
    ...
    print(f"[方案二-RRT2D] checkpoint 模型保存目录: {MODEL_SAVE_DIR}/")
    print(f"[方案二-RRT2D] 第一阶段训练 (mu3=0.0): N_GAMES={N_GAMES}")
    if STAGE == "stage2":
    ...
    if evaluate:
        ...
        print('----training (RRT2D-Augmented) start----')

    evaluator = UAVTaskEvaluator([0, 0, 0], max_time_steps=MAX_STEPS, dones=[False, False, False, False])

    i = 0
    while i < N_GAMES:
        with tqdm(total=BATCH_SIZE, desc=f"Batch Progress (Total {i}/{N_GAMES}) [RRT2D][{STAGE}]", unit="episode") as pbar:
            for _ in range(BATCH_SIZE):
                ...
                writer.add_scalar("UAV Reward (RRT2D)", score, i)
                writer.add_scalar("Target Reward (RRT2D)", score_target, i)
            ...
            pbar.close()
            tqdm.write(f'Batch {i // BATCH_SIZE} completed (RRT2D).')
            ...
            if avg_score > best_score:
                ...
                try:
                    eval_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'evaluate_arvp_rrt2d.py')
                    eval_cmd = [
                    ...
                    try:
                        ...
                        tqdm.write(f"evaluate_arvp_rrt2d.py 完成，exit code={process.returncode}")
    ...
    print(f"[方案二-RRT2D] 奖励历史 CSV 保存到: {csv_path}")
    ...
    hp_rows = [
        ...
        ('mu4_r_finish', 10),
        ('mu5_vo', 0.1),
        ('mu_rrt_total', env.mu_rrt),
        ('mu_rrt_track_alignment', env.mu_rrt_track),
        ('mu_rrt_progress', env.mu_rrt_progress),
        ('rrt_robot_radius', env.rrt_robot_radius),
        ('rrt_max_iter', env.rrt_max_iter),
        ('rrt_step_size', env.rrt_step_size),
        ('rrt_goal_radius', env.rrt_goal_radius),
        ('rrt_neighbor_radius', env.rrt_neighbor_radius),
        ('rrt_encircle_radius', env.rrt_encircle_radius),
        ('rrt_num_waypoints', env.rrt_num_waypoints),
        ('capture_milestone_reward', env.capture_milestone_reward),
        ('capture_d_milestone_1', env.capture_d_milestone_1),
        ('capture_d_milestone_2', env.capture_d_milestone_2),
        ('method', '方案二: RRT* 2D 采样路径作为专家轨迹的演示增强 (Demonstration-Augmented RL)'),
        ('stage', STAGE),
        ('run_prefix', RUN_PREFIX),
    ...
    print(f"[方案二-RRT2D] 超参数 CSV 保存到: {hp_csv}")
    ...
    plt.title(f'ARVP-MADDPG + RRT* 2D Augmented (方案二) - {RUN_PREFIX}_{STAGE} - N={len(score_history)}')
    plt.xlabel('Episode')
