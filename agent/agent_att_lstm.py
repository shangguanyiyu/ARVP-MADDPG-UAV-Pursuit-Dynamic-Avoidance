import torch as T
from model.networks_att_lstm import ActorNetwork, CriticNetwork_1, CriticNetwork_2
import numpy as np
...
class Agent:
    def __init__(self, actor_dims, critic_dims, n_actions, n_agents, agent_idx, chkpt_dir, dim_info, agent_id,
                    ...
                    fc2=128, gamma=0.99, tau=0.01, lstm_hidden_dim=256):
        self.gamma = gamma
        self.tau = tau
        self.n_actions = n_actions
        self.agent_name = 'agent_%s' % agent_idx
        self.actor = ActorNetwork(alpha, actor_dims, fc1, fc2, n_actions,
                                  chkpt_dir=chkpt_dir,  name=self.agent_name+'_actor', lstm_hidden_dim=lstm_hidden_dim)
        self.critic = CriticNetwork_1(beta, critic_dims,
                            fc1, fc2, n_agents, n_actions,
                            chkpt_dir=chkpt_dir, name=self.agent_name+'_critic', dim_info=dim_info, agent_id=agent_id, hidden_dim=128,lstm_hidden_dim=lstm_hidden_dim)
        self.target_actor = ActorNetwork(alpha, actor_dims, fc1, fc2, n_actions,
                                        chkpt_dir=chkpt_dir, 
                                        name=self.agent_name+'_target_actor',lstm_hidden_dim=lstm_hidden_dim)
        self.target_critic = CriticNetwork_2(beta, critic_dims,
                                            fc1, fc2, n_agents, n_actions,
                                            chkpt_dir=chkpt_dir,
                                            name=self.agent_name+'_target_critic', dim_info=dim_info, agent_id=agent_id, hidden_dim=128,lstm_hidden_dim=lstm_hidden_dim)
