...
from maddpg_att import MADDPGWithAttention
...
if __name__ == '__main__':
    ...
    maddpg_agents = MADDPGWithAttention(actor_dims, critic_dims, n_agents, n_actions,
                           fc1=128, fc2=128,
                           alpha=0.01, beta=0.01, scenario='UAV_Round_up', chkpt_dir = 'tmp_first/maddpgwithatt/') # alpha=0.00001, beta=0.00005
        ...
        with tqdm(total=BATCH_SIZE, desc=f"Batch Progress (Total {i}/{N_GAMES})", unit="episode") as pbar:
            for _ in range(BATCH_SIZE):
                ...
                while not any(dones):
                    ...
                    # print(actions)
                    obs_, rewards, dones, collision_info, mul_pos = env.step_2(actions)
            ...
            if avg_score > best_score:
                ...
                maddpg_agents.save_checkpoint()
                process = subprocess.Popen(['python', 'main_evaluate_2.py'])
