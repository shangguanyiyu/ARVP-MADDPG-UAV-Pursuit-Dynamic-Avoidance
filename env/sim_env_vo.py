...
class UAVEnv:
    def __init__(self,length=2,num_obstacle=4,num_agents=4):
        ...
        # Observation dimensions for 3D + VO 立体锥 (方案三: 动作空间投影 + 3D cone observation)
        # Hunter UAV (agent_0,1,2): S_uavi(6) + S_team(6) + S_target(3) + S_obser(32) + S_vo_cones(18) = 65
        ...
        #   S_vo_cones: 3 VO cones x 6d each [axis(3) + cos_half_angle + dist_norm + in_VO_flag] = 18
        # Target UAV: S_uavi(6) + S_obser(32) + S_evade_d(3 distances) + S_vo_cones(18) = 59
        #   Target's VO cones are against 3 hunters (treat hunters as obstacles) + static ones.
        self.observation_space = {
            'agent_0': spaces.Box(low=-np.inf, high=np.inf, shape=(65,)),
            'agent_1': spaces.Box(low=-np.inf, high=np.inf, shape=(65,)),
            'agent_2': spaces.Box(low=-np.inf, high=np.inf, shape=(65,)),
            'target': spaces.Box(low=-np.inf, high=np.inf, shape=(59,))
        }
        # VO 方案三参数:
        self.vo_enabled = True                # 是否做动作空间 VO 投影
        self.vo_r_self = 0.06                 # 自身 UAV 半径 (用于 cone)
        self.vo_r_other_dynamic = 0.06        # 其他动态 UAV 半径
        self.vo_r_obstacle_static = 0.12      # 静态障碍物基础半径 (会叠加字典里的 radius)
    ...
    def step(self, actions):
        ...
        # =====================================================================
        # 方案三: 动作空间投影 (VO velocity-obstacle projection)
        # 在 speed limit 之后、位置更新之前，对每个 agent 的新速度应用 VO 投影，
        # 将落入 VO 锥内的速度投影到锥边界，避免碰撞。
        # =====================================================================
        if self.vo_enabled:
            obstacle_list = [
            ...
            for i in range(self.num_agents):
                ...
                # other agents (dynamic): all agents except self
                other_agents = []
                for j in range(self.num_agents):
                    if j == i: continue
                    r_other = self.vo_r_other_dynamic
                    if j == self.num_agents - 1 and i != self.num_agents - 1:
                        # target slightly smaller radius so hunters can still get close
                        r_other = self.vo_r_other_dynamic * 0.8
                    other_agents.append((
                        np.array(self.multi_current_pos[j], dtype=float),
                        np.array(self.multi_current_vel[j], dtype=float),
                        r_other))
                v_safe, _ = vo_aggregate_obstacles(
                    pos, vel, obstacle_list, other_agents,
                    self.vo_r_self, self.vo_r_other_dynamic, self.vo_r_obstacle_static)
                # Respect velocity cap after projection
                vm = np.linalg.norm(v_safe)
                vmax_cur = self.v_max if (i != self.num_agents - 1) else self.v_max_e
                if vm > vmax_cur and vm > 1e-9:
                    v_safe = v_safe / vm * vmax_cur
                new_velocities.append(v_safe)
            for i in range(self.num_agents):
                self.multi_current_vel[i] = np.array(new_velocities[i], dtype=float)
    ...
    def step_2(self, actions):
        ...
        if self.vo_enabled:
            ...
            for i in range(self.num_agents):
                ...
                if vm > vmax_cur and vm > 1e-9:
                    v_safe = v_safe / vm * vmax_cur
                new_velocities_s2.append(v_safe)
            for i in range(self.num_agents):
                self.multi_current_vel[i] = np.array(new_velocities_s2[i], dtype=float)
    ...
    def get_multi_obs(self):
        ...
        # Pre-compute obstacle list for VO projection / cone observations
        obstacle_list = [
            {'position': list(obs.position), 'radius': float(obs.radius)}
            for obs in self.obstacles
        ]
        ...
        hunter_velocities = [np.array(self.multi_current_vel[k]) for k in range(self.num_agents - 1)]
        target_position = np.array(self.multi_current_pos[-1])
        target_velocity = np.array(self.multi_current_vel[-1])

        # Cache VO cone observations and post-step projected velocity
        self.vo_cone_observations = [None] * self.num_agents
        ...
        for i in range(self.num_agents):
            ...
            # =================================================================
            # 方案三: VO 立体锥观测 (3D cone observation 增强)
            # 对每个智能体，基于当前速度，调用 vo_aggregate_obstacles：
            #   - 计算针对 target/其他猎人/静态障碍物 的 top-3 重要 cones
            #   - 返回 18 维观测向量 + 安全投影后的速度(缓存到 self.vo_projected_vel 供 step 使用)
            # =================================================================
            if i != self.num_agents - 1:
                # HUNTER: 其他动态障碍 = 2个猎人 + 1个目标(逃逸目标)
                other_agents = []
                for k in range(self.num_agents - 1):
                    if k == i: continue
                    other_agents.append((hunter_positions[k], hunter_velocities[k], self.vo_r_other_dynamic))
                # 目标也作为动态 obstacle 参与 VO，但不额外惩罚 (radius 稍小)
                other_agents.append((target_position, target_velocity, self.vo_r_other_dynamic * 0.8))
                _, vo_obs_vec = vo_aggregate_obstacles(
                    pos, vel, obstacle_list, other_agents,
                    self.vo_r_self, self.vo_r_other_dynamic, self.vo_r_obstacle_static)
                self.vo_cone_observations[i] = vo_obs_vec
                S_vo = list(vo_obs_vec)  # dim 18
                single_obs = [S_uavi, S_team, S_obser, S_target, S_vo]
            else:
                # TARGET: 所有猎人都是障碍
                other_agents_target = [
                    (hunter_positions[k], hunter_velocities[k], self.vo_r_other_dynamic)
                    for k in range(len(hunter_positions))
                ]
                _, vo_obs_vec_t = vo_aggregate_obstacles(
                    pos, vel, obstacle_list, other_agents_target,
                    self.vo_r_self, self.vo_r_other_dynamic, self.vo_r_obstacle_static)
                self.vo_cone_observations[i] = vo_obs_vec_t
                S_vo = list(vo_obs_vec_t)  # dim 18
                single_obs = [S_uavi, S_obser, S_evade_d, S_vo]
    ...
    def cal_rewards_dones(self,IsCollied,last_d,last_pos):
        ...
        mu5 = 0.1 # 避障 0.2
        # =====================================================================
        # VO 方案三塑形奖励系数 (不使用 APF, 改为 VO 内惩罚 + 安全余量奖励)
        #   mu6: 速度在 VO 锥内惩罚 (每步一次)
        #   mu7: 距离障碍物安全余量奖励 (鼓励保持距离)
        # =====================================================================
        mu6 = 0.8
        mu7 = 0.1
        d_capture = 0.3
        ...
        # =====================================================================
        # 方案三: VO 塑形奖励
        #   - mu6: 若 vo_cone_observations[i] 中包含 in_VO_flag=1 -> 惩罚
        #   - mu7: 奖励最小障碍物距离 (鼓励保持安全余量)
        # =====================================================================
        if hasattr(self, 'vo_cone_observations') and self.vo_cone_observations is not None:
            for i in range(self.num_agents):
                cones_obs = self.vo_cone_observations[i]
                if cones_obs is not None and len(cones_obs) == 18:
                    # 每个 cone 6 维: axis(3) + cos_half_angle + dist_norm + in_VO_flag
                    penalty_inside = 0.0
                    min_dist_norm = 1.0
                    for k in range(3):
                        base = k * 6
                        in_flag = float(cones_obs[base + 5])
                        d_norm  = float(cones_obs[base + 4])
                        penalty_inside += in_flag
                        if d_norm < min_dist_norm:
                            min_dist_norm = d_norm
                    rewards[i] -= mu6 * penalty_inside
                    # dist_norm 越小表示越靠近障碍物 -> 奖励它变远 (即: 1 - min_dist_norm 的好处更大，
                    # 但这里我们用当前的 min_dist_norm 越大越好奖励)
                    rewards[i] += mu7 * min_dist_norm
