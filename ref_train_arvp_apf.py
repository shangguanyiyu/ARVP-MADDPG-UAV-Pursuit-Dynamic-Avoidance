"""
方案一训练脚本：APF 势场引导的状态观测增强 + 势场塑形奖励 (3D 版本)
对应环境 env/sim_env_rvo.py (真3D环境 + APF 方法)
关键改动：
  - 观测维度：agent 56 (3D), target 50 (3D)
  - 动作维度：3 (ax, ay, az)
  - 奖励：额外加入势场塑形奖励
  - N_GAMES=5000
  - 自动创建 log/ 目录保存 reward_curves.png + score_history.csv + hyperparams.csv
  - 新增：手动输入 stage 参数，目录使用 "当前日期+时间_stage参数_" 前缀
  - 新增：模型 / 日志 / runs 全部放到带日期时间前缀的子目录中
"""
import numpy as np
...
RUN_CONFIG_FILENAME = "_last_arvp_apf_run.json"  # 参数传递文件（根目录下）


def save_run_config(config: dict) -> str:
    """把本次训练的关键路径/参数写到 JSON，供 evaluate_arvp_apf.py 直接读取（参数中心化）"""
...
if __name__ == '__main__':
    ...
    STAGE = DEFAULT_STAGE
    print(f"[方案一-APF] 使用 stage = {STAGE}")

    # 生成日期时间+stage 前缀（全局唯一标识）
    RUN_PREFIX = get_run_prefix(STAGE)
    print(f"[方案一-APF] RUN_PREFIX = {RUN_PREFIX}")

    # ================== 需求3: runs/下创建带前缀的目录，UAV_trainingXX放其中 ==================
    RUNS_PARENT_DIR = os.path.join("runs", f"{RUN_PREFIX}_arvp_apf")
    os.makedirs(RUNS_PARENT_DIR, exist_ok=True)
    RUNS_UAV_BASE = os.path.join(RUNS_PARENT_DIR, "UAV_training")

    # ================== 需求2: log_arvp_apf/下创建带前缀的目录 ==================
    LOG_DIR = os.path.join("log_arvp_apf", f"{RUN_PREFIX}_arvp_apf")
    os.makedirs(LOG_DIR, exist_ok=True)
    print(f"[方案一-APF] 日志目录: {LOG_DIR}/")

    # ================== 需求1: 模型保存在 tmp_avoid_dynamic/maddpgwithatt_apf/日期时间_stage_UAV_Round_up ==================
    BASE_CHKPT_DIR = os.path.join("tmp_avoid_dynamic", "maddpgwithatt_apf")
    os.makedirs(BASE_CHKPT_DIR, exist_ok=True)
    # scenario 会拼在 chkpt_dir 后面，最终模型目录 = BASE_CHKPT_DIR / {RUN_PREFIX}_UAV_Round_up
    # RUN_PREFIX 已包含 ts + stage（例: 0814_153020_stage1），此处不再重复拼 stage
    ...
    # ================== 参数中心化：保存 JSON 配置供 evaluate_arvp_apf.py 直接读取 ==================
    # 所有路径/超参数只在这里指定一份，evaluate 端不做任何硬编码，保证一致性
    RUN_CONFIG = {
        # --- 路径相关 ---
        "stage": STAGE,
        "run_prefix": RUN_PREFIX,
        "scenario_name": SCENARIO_NAME,
        "base_chkpt_parent": BASE_CHKPT_DIR,          # tmp_avoid_dynamic/maddpgwithatt_apf
        "chkpt_dir_with_sep": CHKPT_DIR,              # 末尾带分隔符
        "model_save_dir": MODEL_SAVE_DIR,             # 完整模型目录路径
        "log_dir": LOG_DIR,                           # log_arvp_apf/前缀_arvp_apf
        "runs_parent_dir": RUNS_PARENT_DIR,           # runs/前缀_arvp_apf
        "reward_png_name": f"{RUN_PREFIX}_{STAGE}_arvp_apf_reward.png",
        # --- 模型维度（evaluate 端要匹配）---
        "obs_agt_dim": 56,
        "obs_tar_dim": 50,
        "n_agents": 4,
    ...
    print(f"[方案一-APF] actor_dims = {actor_dims}, critic_dims = {critic_dims}")
    ...
    with open(hyperparams_txt_path, 'w') as f:
        ...
        f.write(f'reward components: mu1=0.9, mu2=0.2, mu3=0.0(stage1), mu4=10, mu5=0.1, mu6=0.5(APF shaping), mu7=0.1(APF align)\n')
    print(f"[方案一-APF] 超参数文本保存到: {hyperparams_txt_path}")

    memory = PERMultiAgentReplayBuffer(1000000, critic_dims, actor_dims,
                                       n_actions, n_agents, batch_size=256)

    BATCH_SIZE = 100
    N_GAMES = 5000  # 方案一：严格 5000 episode
    MAX_STEPS = 110
    total_steps = 0
    score_history = []
    ...
    print(f"[方案一-APF] checkpoint 模型保存目录: {MODEL_SAVE_DIR}/")
    print(f"[方案一-APF] 第一阶段训练 (mu3=0.0): N_GAMES={N_GAMES}")
    # maddpg_agents.load_checkpoint()  # 第一阶段从零训练，不加载旧权重

    if evaluate:
        maddpg_agents.load_checkpoint()
        print('----  evaluating  ----')
    else:
        print('----training (APF-Augmented) start----')

    evaluator = UAVTaskEvaluator([0, 0, 0], max_time_steps=MAX_STEPS, dones=[False, False, False, False])

    i = 0
    while i < N_GAMES:
        with tqdm(total=BATCH_SIZE, desc=f"Batch Progress (Total {i}/{N_GAMES}) [APF][{STAGE}]", unit="episode") as pbar:
            for _ in range(BATCH_SIZE):
                ...
                writer.add_scalar("UAV Reward (APF)", score, i)
                writer.add_scalar("Target Reward (APF)", score_target, i)
            ...
            pbar.close()
            tqdm.write(f'Batch {i // BATCH_SIZE} completed (APF).')
            ...
            if avg_score > best_score:
                ...
                # ========== 修复：改为调用 evaluate_arvp_apf.py（参数通过 _last_arvp_apf_run.json 自动读取）==========
                # 不再调用 evaluate_arvp.py（它指向旧目录 tmp_avoid_dynamic/maddpgwithatt/ 且维度 26/23）
                # 使用 --headless --max-frames 50 做快速评估（跑完自动退出，无需 sleep 等待）
                try:
                    eval_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'evaluate_arvp_apf.py')
                    eval_cmd = [
                        sys.executable, eval_script,   # 使用训练时同一个 Python 解释器（避免 numpy/torch 环境不匹配）
                        '--headless',          # 不弹 GUI，跑完就退出
                        '--from-config',       # 优先从 _last_arvp_apf_run.json 读取路径参数
                    ...
                    try:
                        ...
                        tqdm.write(f"evaluate_arvp_apf.py 完成，exit code={process.returncode}")
    ...
    print(f"[方案一-APF] 奖励历史 CSV 保存到: {csv_path}")
    ...
    print(f"[方案一-APF] 超参数 CSV 保存到: {hp_csv}")

    # 3) 需求1+需求2：绘制并保存 reward 曲线
    #    - 文件名：当前日期+时间_stage参数_arvp_apf_reward.png
    ...
    plt.title(f'ARVP-MADDPG + APF Augmented (方案一) - {RUN_PREFIX}_{STAGE} - N={len(score_history)}')
    plt.xlabel('Episode')
